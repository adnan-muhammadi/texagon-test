    ...
    const saveDrugDataToDB = async (payload: any) => {
      try {
        ...
        if (response?.statusCode === SUCCESS_STATUS_CODE) {
          ...
          if ((onButtonSaveData || currentBtnAction == CONSTANTS.PUBLISH) && !scrolledDataSaved) {
          ...
          if (
            ...
            if (currentBtnAction == CONSTANTS.PUBLISH) {
              ...
              // Extract the taskId from the created task data
              const { taskId } = taskCreateData || {};
              ...
              window.history.replaceState(
                null,
                '',
                `/${CONSTANTS.EDIT_DRUG}/${drugId}?taskId=${
                  taskId || taskDetails?.taskId
                }`,
              );
